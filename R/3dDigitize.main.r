...
read.vertex.3D <- function(content, key) {
  ...
  if(numbers[1] == "") {
    print("no surface data, skipping")
    return(-1)
  }
...
switchTab <- function(e, id) {
  ...
  if (id == 0) {
    ...
    if((length(e$dsFiles) != 0) && (class(e) == "surface") && (!is.null(unlist(e$dsFiles[[e$currImgId]]))))
       add("downsample", e$dsFiles[[e$currImgId]], e$currImgId - 1)
...
createMenu <- function(e) {
  ...
  tkadd(fileMenu, "command", label = "Exit", command = function() onExit(e))
...
onExit <- function(e)
{
  if(length(e$dsFiles) != 0)
  {
    for(i in 1:length(e$dsFiles))
      if(!is.null(unlist(e$dsFiles[i])))
      {
        file.remove(e$dsFiles[[i]])
        e$dsFiles[[i]] = NULL
      }
...
onNext <- function(e) {
  ...
  #set("specimen", "reload", e$currImgId-1)

  if(e$indicator == 1)
  {
    draw.digitize(e$currImgId, e$activeDataList[[e$currImgId]][[1]], e$activeDataList[[e$currImgId]][[10]])
    if(nCurrA != 0)
      draw.anchors(e$currImgId, e$activeDataList[[e$currImgId]][[11]])
  }
  else {
    #set("specimen", "allocate", length(e$activeDataList))
    add("specimen", e$activeDataList[[e$currImgId]][[1]], e$currImgId-1)
  }
...
loadPly <- function(e) {
  ...
  if (nSpecimens != 0) {
    ...
    if(nSpecimens > 0) {
      ...
      set("specimen", "allocate", length(e$activeDataList)) #allocates memory for the 1st specimen loaded

      add("specimen", dgtDataList[[1]][[1]], e$currImgId-1) #utilizes ogl_loadModel to load specified .ply file
    }
  }
}
...
drawElements <- function(e, digitize, surfaceData, curves, anchors) {
  ...
  if(!anyNA(anchors) && !is.null(anchors)){
    ...
    print("Missing anchors detected")
    for(i in 2:4)
      tcl(e$nb, "tab", i, state = "normal")
  ...
  for(i in 1:nSpecimens){
    ...
    dgtDataList[[specId]] <- list(specimens[[i]], 0.01, e$landmarkNum, list(), tmpt[i], c(0, 0), 0, surfaces[,,i], anchorNum, landmarks, anchor)
    specId <- specId+1
  }
  ...
  e$activeDataList <- dgtDataList
  draw.digitize(1, e$activeDataList[[1]][[1]], e$activeDataList[[1]][[10]])
